...
class LowCountAccelerationStructure {
public:
    using                Pt                           = TCell::Pt;
    ...
    virtual PI           recommended_nb_threads       () const;
    virtual int          for_each_cell                ( const TCell &base_cell, const std::function<void( TCell &cell, int num_thread )> &f, int max_nb_threads = 0 ); ///< return the first non null value of `f( cell, num_thread )`, or 0. If max_nb_threads = 0, we take recommended_nb_threads() threads
