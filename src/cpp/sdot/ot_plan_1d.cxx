...
// T_T void ot_plan_1d_backward( TensorView<const T,1,Cpu> grad_distance, TensorView<const T,2,Cpu> grad_barycenters, TensorView<const T,2,Cpu> barycenters, TensorView<const T,2,Cpu> potentials, TensorView<const T,3,Cpu> cuts, BatchOfSumOfWeightedDiracs1d<const T,Cpu> diracs, BatchOfPiecewiseAffineGrid1d<const T,Cpu> functions, BatchOfSumOfWeightedDiracs1d<T,Cpu> grad_diracs, BatchOfPiecewiseAffineGrid1d<T,Cpu> grad_functions ) {
//     parallel_for<PI>( 0, ASSERTED_EQUAL( diracs.nb_rows(), functions.nb_rows() ), [&]( PI r ) {
//         ot_plan_1d_backward( grad_distance.row( r ), grad_barycenters.row( r ), barycenters.row( r ), potentials.row( r ), cuts.row( r ), diracs.row( r ), functions.row( r ), grad_diracs.row( r ), grad_functions.row( r ) );
//     });
// }
...
// // -------------------------------------- CUDA  --------------------------------------
// #ifdef __CUDACC__
// T_T void ot_plan_1d( BatchOfSumOfWeightedDiracs1d<const T,Cuda> diracs, BatchOfAffine1d<const T,Cuda> points, TensorView<T,1,Cuda> distances, TensorView<T,2,Cuda> barycenters, TensorView<T,2,Cuda> potentials, TensorView<T,3,Cuda> cuts ) {
...
// T_T void ot_plan_1d_backward( TensorView<const T,1,Cuda> grad_distance, TensorView<const T,2,Cuda> grad_barycenters, TensorView<const T,2,Cuda> barycenters, TensorView<const T,2,Cuda> potentials, TensorView<const T,3,Cuda> cuts, BatchOfSumOfWeightedDiracs1d<const T,Cuda> diracs, BatchOfAffine1d<const T,Cuda> points, BatchOfSumOfWeightedDiracs1d<T,Cuda> grad_diracs, BatchOfAffine1d<T,Cuda> grad_functions ) {
//     using TF = typename IntermediateScalarType<T,Cuda>::type;
