...
def as_jax_ffi_compatible_args( obj ) -> list[ tuple[ any, str, str ] ]:
    """Decompose *arg* into the flat list of jax-compatible Python objects
    ...
    if callable( getattr( obj, "as_jax_ffi_compatible_args", None ) ):
        return obj.as_jax_ffi_compatible_args()
    ...
    # driver
    conv = driver.as_jax_ffi_compatible_args( obj )
    if conv:
        return conv
    ...
    for name, _ in collect_attributes( obj ):
        out += as_jax_ffi_compatible_args( getattr( obj, name ) )
    return out
