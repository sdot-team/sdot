...
class JaxDriver:
    ...
    def tn( self, tensor, ndim ):
        ...
        if self.device is not None:
             res = jax.device_put( res, self.device )
    ...
    def batch_of_ot_plan_for_Piecewise1dAffineFunctions( self, f: sdot.BatchOfSumOfWeightedDiracs, g: sdot.BatchOfPiecewise1dAffineFunctions ) -> BatchOfOtPlans:
        ...
        def sdot_op( dirac_xs, dirac_ws, point_xs, point_ys ):
            def forward_callback( d_xs, d_ws, p_xs, p_ys ):
                ...
                sdot_bindings_cpu.ot_plan_to_piecewise_affine_1d( d_xs, d_ws, p_xs, p_ys, distances, barycenters )

                return distances, barycenters

            res_distances, res_barycenters = jax.pure_callback(
                forward_callback,
            ...
            return res_distances, res_barycenters

        def sdot_op_fwd( dirac_xs, dirac_ws, point_xs, point_ys ):
            distances, barycenters = sdot_op( dirac_xs, dirac_ws, point_xs, point_ys )
            # We save the inputs and the barycenters for the backward pass
            return ( distances, barycenters ), ( dirac_xs, dirac_ws, point_xs, point_ys, barycenters )

        def sdot_op_bwd( res, grads ):
            dirac_xs, dirac_ws, point_xs, point_ys, barycenters = res
            grad_distance, grad_barycenters = grads

            def backward_callback( g_dist, g_bary, d_xs, d_ws, p_xs, p_ys, bary ):
                ...
                sdot_bindings_cpu.backward_ot_plan_to_piecewise_affine_1d(
                    g_dist, g_bary, d_xs, d_ws, p_xs, p_ys, bary,
        ...
        distances, barycenters = sdot_op( f._nd_positions(), f.weights, g.xs, g.ys )
