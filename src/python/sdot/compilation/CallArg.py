...
from sdot.driver import driver
...
class CallArg:
    ...
    # io_category: int # 0 => pure input, 1 => mutable, 2 => return, 3 => workspace
    ...
    def factory( python_class, python_value, io_category: int ):
        ...
        if python_value is not None and callable( getattr( python_value, "call_arg_factory", None ) ):
            return python_value.call_arg_factory( io_category )
        ...
        if callable( getattr( python_class, "call_arg_factory", None ) ):
            return python_class.call_arg_factory( python_value, io_category )

        # arrays
        if driver.is_a_tensor( python_value ):
            from .CallArg_Tensor import CallArg_Tensor
            return CallArg_Tensor.factory( python_class, python_value, io_category )
        ...
        return CallArg_Aggregate.factory( python_class, python_value, io_category )
...
#     self.struct_info = CallArg.struct_info_for( ndim, python_value.dtype, represents_a_dynamic_axis, driver, ct_axes )
...
#         ffi_input = fai.add_input_tensor( python_value, driver, axis_names = axis_names, ct_axes = ct_axes, valid = valid, represents_a_dynamic_axis = represents_a_dynamic_axis )
...
#     self.struct_info = CallArg.struct_info_for( len( shape ), dtype, represents_a_dynamic_axis, driver, ct_axes )
...
# @staticmethod
# def struct_info_for( ndim, dtype, represents_a_dynamic_axis, driver, ct_axes ) -> StructInfo:
#     if represents_a_dynamic_axis:
#         return StructInfo( name = "DynamicAxis", params = { str( ndim ): "", "Arch": "typename" }, include = "sdot/support/DynamicAxis.h", includes = set(), avoid_params_in_inst = [] )
...
#     params[ ndtype ] = "typename"
#     params[ str( ndim ) ] = ""
#     params[ "Arch" ] = "typename"

#     avoid_params_in_inst = []
#     for ct_axis in ct_axes:
#         c, o, n = CallArg.axis_analysis( ct_axis )
#         n = "ct_" + n + "_"
#         avoid_params_in_inst.append( n )
#         params[ n ] = "int"

#     return StructInfo( name = "TensorView", params = params, include = "sdot/support/TensorView.h", includes = set(), avoid_params_in_inst = avoid_params_in_inst )
...
#     self.struct_info.include = f"sdot/generated_includes/{ python_value.__class__.__name__ }.h"
...
#         for param_name, param_kind in sc.struct_info.params.items():
#             if param_kind:
#                 self.struct_info.add_param( param_name, param_kind )

#         if sc.struct_info.include:
#             self.struct_info.includes.add( sc.struct_info.include )
#         for include in sc.struct_info.includes:
#             self.struct_info.includes.add( include )
...
#     self.struct_info.include = f"sdot/generated_includes/{ return_type.__name__ }.h"
#     if self.struct_info.name not in fai.aggregates:
#         fai.aggregates[ self.struct_info.name ] = self
...
#                         ct_axes[ n ] = ct_axis_limit
#             for ct_axis_name, ct_axis_limit in ct_axes.items():
#                 c, o, n = CallArg.axis_analysis( ct_axis_name )
#                 value = getattr( self.python_ctor, n, None )
#                 if value is None:
#                     if not isinstance( self.python_value, ( Return, Workspace ) ):
#                         raise RuntimeError( f"expecting a return of an object with a '{ n }' property for '{ self.python_value }" )
#                     value = self.python_value.type_kwargs[ n ]
#                 lst.append( f".ct_{ n } = CtInt<{ value }>()" )
...
#     ct_axes = {} # name -> limit
#     for item in self.sub_list:
#         for ffi in [ item.ffi_output, item.ffi_input ]:
#             if ffi:
#                 for ct_axis_name, ct_axis_limit in ffi.ct_axes.items():
...
#         for ffi in [ item.ffi_output, item.ffi_input ]:
#             if ffi:
#                 for num_axis, axis_name in enumerate( ffi.axis_names ):
...
#         lines.append( f"template<{ str.join( ",", [ kind + " " + name for name, kind in params.items() if kind ] ) }>" ) #
#     lines.append( f"struct { self.struct_info.name } {{" )

#     # attributes
#     for item in self.sub_list:
#         struct_name = item.struct_info.name

#         params = []
#         for key, kind in item.struct_info.params.items():
#             if key in item.struct_info.avoid_params_in_inst:
#                 continue
#             params.append( key )
#         struct_name += f"<{ str.join( ",", params ) }>" #  if kind

#         lines.append( f"    { struct_name } { item.attribute_name };" )

#         if item.struct_info.include:
#             includes.add( item.struct_info.include )
#         for include in item.struct_info.includes:
