...
struct LocBspBindings {
    ...
    static void add_mod( auto &m ) {
        ...
        nb::class_<BspType>( m, m0.c_str() )
            .def( "__init__", []( BspType *self, nb::ndarray<const TF> all_the_paths, nb::ndarray<const TF> min_max_pts, nb::ndarray<const PI> indices, nb::ndarray<const TF> points, nb::ndarray<const TF> path, PI max_points_per_cell ) {
                new ( self ) BspType(
